      ...
      tableFmt() {
        if ($.fn.bootstrapTable) {
          ...
          $.fn.bootstrapTable.columnDefaults.formatter = function(value, row, index) {
            ...
            var divDom = $(
              "<div style='white-space: nowrap;display:inline-block;overflow: hidden;word-break: break-all;word-wrap: break-word;text-overflow: ellipsis;font-size: 1.2rem;' title='" +
              value + "'>" + value + "</div>")
    ...
    ztree(type, id, data, seeting) {
      ...
      let ztree = {
        ...
        addSeach() {
          ...
          //搜索框
          $(id).before(
            `<input type="text" placeholder="请输入搜索关键字~" style="width:100%" class="form-control rj_ztree_seach">`
          )
          /*
          ...
          ztreeRadio: {
            ...
            callback: {
              ...
              onCheck: function(e, treeId, treeNode) {
                ...
                if (treeNode.checked) {
                  $("#" + treeNode.tId).closest(".ztree").closest("div").fadeOut("fast").prev().val(html).prev()
                    .val(treeNode.id)
                } else {
          ...
          ztreeCheckbox: {
            ...
            callback: {
              ...
              onCheck: function(e, treeId, treeNode) {
                ...
                $("#" + treeNode.tId).closest(".ztree").closest("div").prev().val(len > 0 ? `(${len}个)${html}` :
                  "").prev().val(ids);
              }
            }
          }
              ...
              for (let i in data) {
                ...
                $itemHtml.find(".rj_toggleRow_input").each((inputIndex, inputDom) => {
                  if (data[i][this.opts.defaultKey[inputIndex]] != undefined) {
                    ...
                    console.error(
                      `rj.toggleRow组件 ${this.opts.defaultKey[inputIndex]}:在数据源的key与name为该值的表单控件没有对应项`)
                  }
                });
