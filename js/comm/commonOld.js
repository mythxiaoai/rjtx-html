...
(function(win){
	...
	win.openWin=function(url,title,callback){
		...
		var index=layer.open({
			...
			fixed: false, //不固定
			maxmin: parm.maxmin||false,//显示最大化窗口
			shadeClose:false,
			//点击窗外关闭
			content: parm.content,
			moveOut:true,//可拖拽到视口外
			zIndex:parm.zIndex||999,
			cancel:parm.cancel||undefined,
			scrollbar:false,
...
(function(win){
	...
	win.xalert=function(title,text,type,fn){
		var json =getDefault();
			json.title=title;
			json.text=text;
			json.type=type || "info",
	...
	win.xerror=function(title,text,type,fn){
		var json =getDefault();
		json.title=title;
		json.text=text;
	...
	win.xconfirm=function(title,text,type,fn){
		var json =getDefault();
		json.title=title;
		json.text=text;
	...
	win.socket={
		...
		linked:function(userId){
			if('WebSocket' in win){
			  ...
			  var bpath=localhostPaht+project;
		      bpath = bpath.substr(bpath.indexOf("://"));
		      bpath = "ws"+bpath;
	          win.websocket = new WebSocket(bpath+'/loginSocket/'+userId);
	      }else{
